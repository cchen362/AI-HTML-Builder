...
def test_export_html_returns_200(client, mock_export_fns):
    ...
    resp = client.post("/api/sessions/sess-1/documents/doc-123/export/html", params={"title": "test"})
...
def test_export_html_with_version(client, mock_export_fns):
    mock_export, _mock_formats = mock_export_fns
    mock_export.return_value = _html_result()
    resp = client.post("/api/sessions/sess-1/documents/doc-123/export/html", params={"version": 2})
    assert resp.status_code == 200
...
def test_export_pdf_returns_200(client, mock_export_fns):
    ...
    resp = client.post("/api/sessions/sess-1/documents/doc-123/export/pdf")
    assert resp.status_code == 200
...
def test_export_returns_400_on_export_error(client, mock_export_fns):
    ...
    resp = client.post("/api/sessions/sess-1/documents/doc-123/export/html")
...
def test_content_disposition_has_filename(client, mock_export_fns):
    mock_export, _mock_formats = mock_export_fns
    mock_export.return_value = _html_result()
    resp = client.post("/api/sessions/sess-1/documents/doc-123/export/html", params={"title": "myfile"})
