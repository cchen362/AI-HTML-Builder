# Implementation Plan 008: Deployment and Security

## Status: PENDING (Ready to implement)

**Prerequisites met:** Plans 001-007, 009a, 009b all COMPLETE. 244+ tests passing.

## ERRATA — Known Issues in This Plan (must correct during implementation)

This plan was written before Plans 001-007 were implemented. The following discrepancies exist between this document and the actual codebase:

1. **SSE, not WebSocket**: The app uses SSE + HTTP POST (not WebSocket). References to "WebSocket" in NPM config (Phase 2 Step 2.1 item 4, Phase 2 Step 2.2 CSP header, verification checklist) are incorrect. WebSocket support in NPM is harmless but unnecessary; the CSP `connect-src` should NOT include `ws: wss:`.
2. **CSP header needs Google Fonts**: The Content-Security-Policy in Phase 2 Step 2.2 must allow `fonts.googleapis.com` and `fonts.gstatic.com` (added in Plan 009a for Bricolage Grotesque, IBM Plex Sans, JetBrains Mono).
3. **`config.py` already exists**: Step 4.2 proposes a full rewrite of `config.py` — the actual file is lean (32 lines) and correct. Do NOT replace it wholesale. Only add fields that are truly needed (e.g., `environment`, `cors_origins`). Many proposed fields (`max_claude_iterations`, `max_html_context_length`, cost pricing, frontend/backend URLs, export dir/retention) are not in the current config and may not be needed.
4. **`aiosqlite` has no `execute_fetchone`/`execute_fetchall`**: The cost tracker code in Step 5.1 uses these non-existent methods. Use `cursor = await db.execute(); row = await cursor.fetchone()` pattern instead.
5. **Model identifiers outdated**: Cost tracker references `claude-sonnet-4` — actual model is `claude-sonnet-4-5-20250929`. Image model is `gemini-2.0-flash-preview-image-generation`, not `nano-banana-pro`.
6. **Rate limiter already exists**: `backend/app/utils/rate_limiter.py` already exists (from Plan 001). Step 6.1 proposes creating `middleware/rate_limiter.py` — review existing implementation before adding middleware.
7. **Lifespan pattern, not `on_event`**: `main.py` uses the FastAPI lifespan pattern (not deprecated `@app.on_event`). Background tasks (cleanup, etc.) should be added to the existing lifespan context manager.
8. **CostsPage uses external dependency**: Step 5.3 imports `react-chartjs-2` — this is a new npm dependency not currently installed. Consider whether a chart library is needed for 2-5 users, or use a simpler text-based cost display.
9. **Cost tracking table already exists**: `database.py` already defines `cost_tracking` table. The schema in Step 5.1 differs (extra columns: `username`, `session_id`, `images_generated`, `request_type`). Reconcile with existing schema.
10. **Deploy script says "Enable WebSockets"**: Line in deploy output should say "SSE streaming (no WebSocket config needed)".
...
- [ ] Plans 001-007 are fully completed and tested
- [ ] Plans 009a-009b are fully completed and tested
- [ ] SQLite database implementation is working (aiosqlite + WAL mode)
- [ ] All API endpoints are functional (244+ tests passing)
- [ ] Frontend build works without errors (TypeScript clean, Vite build clean)
...
- [ ] You have read the ERRATA section above and will correct discrepancies during implementation

**ESTIMATED EFFORT**: 2-3 days
**DEPENDENCIES**: Plans 001-007, 009a, 009b
...
4. Fill in the **Details** tab:
   ...
   - **Websockets Support**: Optional (app uses SSE, not WebSocket — harmless to enable)
...
add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: blob:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self'; frame-ancestors 'self';" always;
...
echo "  4. (Optional) Enable WebSockets Support — app uses SSE, not WebSocket"
...
- [ ] **NPM Proxy**: Forwarding to port 8080, SSE streaming works (WebSocket support optional)
